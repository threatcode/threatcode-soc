...
import (
  ...
  "github.com/threatcode/threatcode-soc/packet"
...
func (datastore *FileDatastoreImpl) GetPacketStream(ctx context.Context, jobId int, unwrap bool) (io.ReadCloser, string, int64, error) {
  ...
  if job != nil {
    if datastore.jobIsAllowed(ctx, job, "read") {
      if job.Status == model.JobStatusCompleted {
        filename = fmt.Sprintf("threatsensor_%s_%d.%s", sanitize.Name(job.GetNodeId()), job.Id, sanitize.Name(job.FileExtension))
        var file *os.File

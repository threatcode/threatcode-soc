FROM ghcr.io/threatcode/golang:1.20.1-alpine as builder
ARG VERSION=0.0.0
RUN apk update && apk add libpcap-dev bash git musl-dev gcc npm python3 py3-pip py3-virtualenv
COPY . /build
WORKDIR /build
RUN mkdir gitdocs && cd gitdocs && \
	git clone --no-single-branch --depth 50 https://github.com/threatcode/securityonion-docs.git . && \
...
RUN mkdir -p /opt/threatsensor/jobs && chown socore:socore /opt/threatsensor/jobs
RUN mkdir -p /opt/threatsensor/logs && chown socore:socore /opt/threatsensor/logs
WORKDIR /opt/threatsensor
COPY --from=builder /build/threatsensor .
...
COPY --from=builder /build/threatsensor.json .
...
RUN bash -c "[[ $VERSION == '0.0.0' ]]" || \
    wget https://github.com/threatcode/securityonion-docs/raw/$(echo $VERSION | cut -d'.' -f 1,2)/images/cheat-sheet/Security-Onion-Cheat-Sheet.pdf -O html/docs/cheatsheet.pdf
...
USER socore
EXPOSE 9822/tcp
ENTRYPOINT ["/opt/threatsensor/threatsensor"]
